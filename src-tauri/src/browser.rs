...
impl Browsers {
    fn browser_app_names(browser: &str) -> Browsers {
        match browser.to_lowercase().as_str() {
            ...
            | "google chrome.app"
            | "com.google.chrome"
            | "google-chrome.desktop" => Browsers::Chrome,
            "microsoft-edge"
            | "edge"
            | "msedge"
            | "msedge.exe"
            ...
            | "firefox.app"
            | "org.mozilla.firefox"
            | "firefox.desktop" => Browsers::Firefox,
...
fn get_default_browser_macos() -> Option<String> {
    ...
    let output_str = String::from_utf8_lossy(&output.stdout);
    let re =
        Regex::new(r#""LSHandlerURLScheme" = "http";\s*"LSHandlerRoleAll" = "([^"]+)";"#).unwrap();
    re.captures(&output_str)
    ...
    match Browsers::browser_app_names(browser) {
        ...
        Browsers::Safari => (
            ...
            vec![
                ...
                String::from("Safari"),
                String::from("--new"),
                format!("--args --window-name={}", window_title),
            ],
        ),
